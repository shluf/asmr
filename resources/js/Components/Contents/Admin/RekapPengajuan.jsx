...
const RekapPengajuan = ({ select }) => {
  ...
  const [rekapSurat, setRekapSurat] = useState([]);

  useEffect(() => {
    fetchRekapPengajuanData(setRekapSurat);
  }, []);
        ...
        {!rekapSurat.data ? (
          ...
          <CardContent className="p-6">
            ...
            <div className='flex flex-col h-full justify-between'>
              ...
              <p className="text-sm flex h-1/2 text-blue">Semua pengajuan surat telah diproses</p>
            </div>
          </div>
          </CardContent>
        </Card>
        ) : (
          rekapSurat.data.map((surat, index) => (
            <Collapsible
